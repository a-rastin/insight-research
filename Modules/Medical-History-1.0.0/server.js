...
function validateV2Write(body, creating) {
  const errors = [];
  const allowed = new Set(creating
  ...
  Object.keys(body).filter((field) => !allowed.has(field)).forEach((field) => errors.push(`${field} is not allowed`));
  ...
  if (!Array.isArray(body.pastMedicalHistory) || body.pastMedicalHistory.some((value) => !COMORBIDITY_OPTIONS.includes(value))) errors.push("pastMedicalHistory must contain only supported options");
  if (!Array.isArray(body.medications) || body.medications.length > 20) {
    ...
    body.medications.forEach((medication, index) => {
      if (!medication || typeof medication.originalText !== "string" || !medication.originalText.trim() || medication.originalText.length > 500) errors.push(`medications[${index}].originalText is required`);
      const normalized = medication?.normalizedIdentity;
      if (!normalized || !NORMALIZATION_STATES.includes(normalized.state)) errors.push(`medications[${index}].normalizedIdentity.state is invalid`);
      if (normalized?.state === "matched" && (!normalized.conceptId || !normalized.display)) errors.push(`medications[${index}].normalizedIdentity requires conceptId and display when matched`);
      if (normalized?.state !== "matched" && (normalized?.conceptId != null || normalized?.display != null || normalized?.terminologyVersion != null)) errors.push(`medications[${index}].normalizedIdentity identifiers are allowed only when matched`);
    });
  }
  ...
  if (!body.actor || !isUuid(body.actor.actorId) || body.actor.role !== "psychiatrist") errors.push("actor must contain a UUID actorId and psychiatrist role");
...
async function createV2Assessment(req, res, context) {
  ...
  if (prior) {
    ...
    const assessment = store.assessments[prior.assessmentId];
  ...
  store.idempotency[key] = { fingerprint, assessmentId: assessment.assessmentId, createdAt: assessment.createdAt };
