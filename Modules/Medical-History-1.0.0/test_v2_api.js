...
(async () => {
  try {
    ...
    const preflight = await fetch(`${base}/api/medical-history/v2/assessments/${patientId}`, {method: "OPTIONS"});
    assert.match(preflight.headers.get("access-control-allow-methods"), /PUT/);

    const openapi = JSON.parse(fs.readFileSync(path.join(__dirname, "contracts", "openapi-v2.json"), "utf8"));
    assert.deepEqual(Object.keys(openapi.paths).sort(), [
      "/api/medical-history/v1/contract",
      "/api/medical-history/v2/assessments",
      "/api/medical-history/v2/assessments/{assessmentId}",
      "/api/medical-history/v2/contract/document",
      "/api/medical-history/v2/contract/openapi",
      "/api/medical-history/v2/contract/schema",
      "/healthz",
      "/readyz"
    ]);
    ...
    const replay = await fetch(`${base}/api/medical-history/v2/assessments`, {
      ...
      headers: {...headers, "Idempotency-Key": "medical-history-case-0001"},
      body: JSON.stringify({
        actor: {role: "psychiatrist", actorId},
        ...
        medications: [{
          normalizedIdentity: {terminologyVersion: null, display: null, conceptId: null, state: "unresolved"},
          frequencyText: "daily",
          routeText: "oral",
          doseText: "300 mg",
          originalText: "Lithium 300 mg oral daily"
    ...
    const invalidCases = [
      {...body, pastMedicalHistory: ["Hypertension", "Hypertension"]},
      {...body, medications: [{...body.medications[0], doseText: 300}]},
      {...body, medications: [{...body.medications[0], normalizedIdentity: {...body.medications[0].normalizedIdentity, extra: true}}]},
      {...body, priorAntipsychoticTherapy: "no", priorAntipsychoticTherapySuccessful: "yes", antipsychotic: null},
      {...body, actor: {...body.actor, displayName: "not accepted"}}
    ];
    for (const [index, invalidBody] of invalidCases.entries()) {
      const invalid = await fetch(`${base}/api/medical-history/v2/assessments`, {
        method: "POST",
        headers: {...headers, "Idempotency-Key": `medical-history-invalid-${index}`},
    ...
    const replayAfterUpdate = await fetch(`${base}/api/medical-history/v2/assessments`, {
      method: "POST",
      headers: {...headers, "Idempotency-Key": "medical-history-case-0001"},
      body: JSON.stringify(body)
    });
    assert.deepEqual(await replayAfterUpdate.json(), assessment);
    ...
    const storePath = path.join(dataDir, "medical_history_assessments_v2.json");
    const store = JSON.parse(fs.readFileSync(storePath, "utf8"));
    store.idempotency["medical-history-case-0001"].createdAt = "1970-01-01T00:00:00.000Z";
    fs.writeFileSync(storePath, JSON.stringify(store));
    const afterRetention = await fetch(`${base}/api/medical-history/v2/assessments`, {
      method: "POST",
      headers: {...headers, "Idempotency-Key": "medical-history-case-0001"},
      body: JSON.stringify({...body, substantialSuicideRisk: "no"})
    });
    assert.equal(afterRetention.status, 201);
    assert.notEqual((await afterRetention.json()).assessmentId, assessment.assessmentId);
